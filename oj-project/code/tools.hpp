...
class FileUtil
{
    public:
        ...
        /*
 	* 将ReadFile设置为static函数就能通过类名::成员函数名就可以调用到这个函数，不需要实例化对象
 	*/ 
	    ...
	    // ifstream文件输入流
            ...
            {
		// getline读到换行就会结束，所以为了我们读到content中的内容也像file中的分行显示，需要在line后边再加上一个\n
                (*content) += line + "\n";
            }
            file.close();
            return true;
...
/*
 * StringUtil类中只封装了一个Boost库中的split函数
 */
class StringUtil
{
public:
...
public:
	//  
        ...
        {
            struct timeval tv; // 出参
	    
	/*
	 * gettimeofday是一个系统调用函数
	 * #include <sys/time.h>
	 *
